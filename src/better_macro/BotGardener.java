...
public class BotGardener extends Globals {
	...
	public static final float MAX_PATCH_SEPARATION = 12;
	
	public static void addGridLocation() throws GameActionException {
		if(plantingList.getLength() > LIST_HAZARD_LENGTH)
			return;

		float r = myRand.nextFloat() * (rc.getType().sensorRadius - GARDENER_PATCH_RADIUS);
		float theta = myRand.nextFloat() * (float)Math.PI * 2;

		Direction dir = new Direction(theta);
		MapLocation location = rc.getLocation().add(dir, r);

		TreeInfo[] trees = rc.senseNearbyTrees();

		int num_spots = getOpenTreeSpotsAbout(location, trees);
		if (num_spots < 3) return;

		MapLocation nearestPlanted = trashList.peakNearest(location);
		if (nearestPlanted != null) {
			float distPlanted = location.distanceTo(nearestPlanted);
			if (distPlanted < GARDENER_PATCH_RADIUS * 2 || distPlanted > MAX_PATCH_SEPARATION) return;
		}
		MapLocation nearestQueued = plantingList.peakNearest(location);
		if (nearestQueued != null) {
			float distQueued = location.distanceTo(nearestQueued);
			if (distQueued < GARDENER_PATCH_RADIUS) return;
		}

		plantingList.addLocation(location);
	}
}
