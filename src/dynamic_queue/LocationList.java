...
public class LocationList extends Globals {
	...
	public final int start_limit, end_limit, length, HEAD_CHANNEL;

	// Locally stored list of locations to add
	public Queue<MapLocation> to_add;
	...
	public LocationList(int _start, int _end) throws GameActionException {
		...
		to_add = new LinkedList<MapLocation>();
	}
	...
	public void pushLocation() throws GameActionException {
		if (!to_add.isEmpty())
			broadcastLocation(to_add.poll());
	}
	
	private void broadcastLocation(MapLocation loc) throws GameActionException
	{
		int head = getHead();
		if(head == 0)
		{
