	...
	public static void end_loop_common() throws GameActionException // things that all robots do at end of loop
	{
		...
		for (TreeInfo tree : trees) {
			if (tree.getContainedRobot() != null)
			{
				...
				System.out.println("Tree Location: " + treeLocation.toString());
				treeList.addLocation(treeLocation);
		...
		for (int i = 0; i < 1; i++) {
			treeList.pushLocation();
			plantingList.pushLocation();
			trashList.pushLocation();
		}

		// If rounds have already changed on us don't yeild the clock
		if (rc.getRoundNum() == currentRound)
			Clock.yield();
		else
			System.out.println("Ran out of bytecode; proceeding to next round");
	}
}
